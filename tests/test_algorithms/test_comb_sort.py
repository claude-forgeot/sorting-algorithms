...
def test_already_sorted(already_sorted):
    assert comb_sort(already_sorted) == already_sorted


def test_reverse_sorted(reverse_sorted):
    assert comb_sort(reverse_sorted) == sorted(reverse_sorted)


def test_with_duplicates(with_duplicates):
    assert comb_sort(with_duplicates) == sorted(with_duplicates)


def test_matches_sorted(unsorted):
    assert comb_sort(unsorted) == sorted(unsorted)


def test_callback_called(unsorted):
    events = []
    comb_sort(unsorted, on_step=lambda arr, i, j, event_type: events.append(event_type))
